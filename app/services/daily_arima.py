...
def get_daily_sales_forecast(consolidated_csv, forecast_periods=10):
    """
    Prophet-based forecast:
    Loads historical daily sales data from the consolidated CSV file,
    cleans and log-transforms the data, fits a Prophet model,
    and returns a forecast for the next forecast_periods days.
    """
    import numpy as np
    from prophet import Prophet

    try:
        # 1) Build the absolute path if necessary
        ...
        # 3) Convert 'Day' to datetime
        df['Day'] = pd.to_datetime(df['Day'], errors='coerce')
        df = df.dropna(subset=['Day'])

        # 4) Prepare data for Prophet: rename columns to ds and y
        df_prophet = df[['Day', 'total sales']].rename(columns={'Day': 'ds', 'total sales': 'y'})

        # 5) Remove zero or negative values
        df_prophet = df_prophet.dropna(subset=['y'])
        df_prophet = df_prophet[df_prophet['y'] > 0]

        # 6) Remove extreme outliers (optional)
        ...
        if df_prophet.shape[0] < 2:
            print("Not enough data for a meaningful forecast after cleaning.")
            return []

        # 8) Log-transform the target
        df_prophet['y'] = np.log(df_prophet['y'])

        # 9) Fit the Prophet model
        model = Prophet(
        ...
        # 10) Create a future DataFrame and forecast
        future = model.make_future_dataframe(periods=forecast_periods)
        forecast = model.predict(future)

        # 11) Inverse the log transform
        forecast['yhat'] = np.exp(forecast['yhat'])

        # 12) Extract forecast for the next forecast_periods days
        forecast_output = forecast[['ds', 'yhat']].tail(forecast_periods)
        forecast_output = forecast_output.rename(columns={'ds': 'Day', 'yhat': 'Total Sales'})
        forecast_output['Total Sales'] = forecast_output['Total Sales'].round(0).astype(int)

        return forecast_output.to_dict(orient='records')

    except Exception as e:
        print(f"[ERROR] get_daily_sales_forecast (Prophet): {e}")
        return []
